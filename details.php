...
if(isset($_POST['act'])){
	mysql_query("update staff set status = case when status < 1 then 1 when status > 0 then 0 end where staffid = '$id'");
	mysql_query("update login set status = case when status < 1 then 1 when status > 0 then 0 end where username = '$id'");
}
...
<tr><td colspan="2"><input type="submit" name="act" value="<?php echo $details['status']==1?"Deactivate Staff":"Activate Staff" ?>" style="background-color:#387070; color:#FFF;"  /></td></tr>
</table>
</form>
</body>
</html>